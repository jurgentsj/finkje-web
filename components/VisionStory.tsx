  ...
  return (
    <section className="overflow-hidden bg-sand px-6 py-24 text-black sm:py-32" aria-label="De visie van Finkje">
      <div className="mx-auto max-w-[1360px]">
        ...
        <div className="relative mb-20 min-h-[180px] py-10 sm:mb-24">
          <div className="absolute top-[93px] left-[-5vw] h-px w-[110vw] bg-black/25" aria-hidden="true" />
          <div className="absolute top-[85px] left-[calc(7.14%*var(--active))] z-10 h-4 w-4 -translate-x-1/2 rotate-45 bg-accent transition-all duration-700" style={{ "--active": active } as React.CSSProperties} aria-hidden="true" />
          <div className="relative grid grid-cols-2 gap-x-6 gap-y-12 sm:grid-cols-4 lg:grid-cols-7 lg:gap-5">
            {chapters.map((item, index) => (
              <button key={item.title} type="button" onClick={() => setActive(index)} className={`text-left transition-opacity ${active === index ? "opacity-100" : "opacity-45 hover:opacity-75"}`} aria-pressed={active === index}>
                <span className="block max-w-[13ch] font-display text-[clamp(20px,2.2vw,30px)] leading-[0.95] font-semibold tracking-[-0.045em]">{item.title}</span>
